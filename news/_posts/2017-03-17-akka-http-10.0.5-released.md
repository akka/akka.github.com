...
Akka HTTP 10.0.x will remain compatible with Akka 2.4.x and Akka 2.5.x during its lifetime, yet it may require the respective latest version of Akka itself. Since Akka is binary compatible in such versions, it is trivial to upgrade both projects in case you need to do so.
